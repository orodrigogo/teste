...
import { metricsProductsViewers } from "../../../http/metrics-products-viewers";

export function MetricsProductViewersCard() {
  const { data, isLoading } = useQuery({
    queryKey: ["metrics", "products-viewers"],
    queryFn: metricsProductsViewers,
    staleTime: 5 * 60 * 1000,
  });
  ...
  return (
    <div className="p-3 rounded-xl bg-white flex items-center gap-4">
      <div className="flex items-center justify-center bg-blue-light rounded-xl w-20 h-[5.375rem]">
        <UserMultipleIcon className="size-10 text-blue-dark" />
      </div>
